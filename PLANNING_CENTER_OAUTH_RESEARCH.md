...
> **Status:** Backend + DB work complete (all of #33–#39). Only `prayerloop-mobile` changes remain. See §Q.
> **Date:** 2026-06-30
>
...
> ✅ **2026-07-09 addendum (§Q): Backend + DB work is done.** All seven tracked issues (#33–#39) are implemented and verified against the project board and repo state. Remaining work is entirely in `prayerloop-mobile`. See §Q.
...
- **EAS + Apple Developer portal ordering (confirmed 2026-07-08):** enabling "Sign In with Apple" on the App ID in the Apple Developer portal alone isn't enough for EAS to know it should regenerate anything. The `"expo-apple-authentication"` plugin above is what tells EAS Build to add the `com.apple.developer.applesignin` entitlement to the provisioning profile — without it in `app.config.ts`/`app.json`, EAS won't detect the mismatch even after the capability is enabled on the portal side. With that plugin present and EAS-managed credentials, the next `eas build -p ios` detects the entitlement mismatch against the existing profile and regenerates it automatically (or prompts if interactive); manual/`credentials.json`-managed credentials need `eas credentials` → regenerate provisioning profile run by hand. `eas submit` doesn't touch profiles at all — only `eas build` does. The portal's "adding/removing capabilities invalidates existing provisioning profiles" warning is boilerplate for any capability change and is expected/safe to click through; it does not affect the already-live App Store binary, only future builds.
...
## P. 2026-07-08 addendum — #38 shipped; OAuthLink has no cross-account email-collision check

### P.1 What shipped

Issue #38 (Phase 2: account linking/unlinking) is implemented in full:

- **`OAuthLink`** (`POST /auth/oauth/:provider/link`, authenticated) — exchanges the code, looks up `(provider, provider_user_id)`: bound to a *different* account → `409`; bound to `currentUser` already → idempotent `200` (tokens refreshed); not found → `INSERT user_external_identity` for `currentUser` (`uq_uei_user_provider` still caps it at one identity per provider per account).
- **`OAuthUnlink`** (`DELETE /auth/oauth/:provider/link`, authenticated) — always operates on `currentUser` from `CheckAuth`, never a client-supplied id. Blocked with `409` when it would remove the account's only sign-in method (`password IS NULL AND identity_count <= 1`); otherwise best-effort revokes the provider token (`services.DecryptToken` + `OAuthProvider.Revoke`, same pattern as `revokeIdentityTokens`) before hard-deleting the row.
- **`ListUserIdentities`** (`GET /users/me/identities`) — returns `{linkedProviders: [...]}`, derived live from `user_external_identity` (never a stored/cached boolean, per §B Q1).
- **`SetPassword`** (`POST /users/me/password`) — sets a first password for an OAuth-only account (`password IS NULL`); no old-password check since the JWT already proves ownership. Rejects with `409` if the account already has a password (use the existing `ChangeUserPassword` instead).
- `models.UserWithLinkedProviders` decorates the `user` object in `OAuthLink`/`OAuthUnlink` responses with `linkedProviders`, computed per-request — matching §B Q1's "derive, don't store" rule.
- All four routes registered in the authenticated group (`main.go`); full `sqlmock`-based test coverage added (`oauthController_test.go`, `userController_test.go`); `go build`/`go vet`/`go test ./...` pass.
- `prayerloop-bruno` collection updated to match: `PCLink`/`PCUnlink`, `GoogleLink`/`GoogleUnlink`, `AppleLink`/`AppleUnlink` (`OAuth/`), `ListUserIdentities`/`SetPassword` (`Users/`).

### P.2 Gap: `OAuthLink` never checks whether the provider identity's email collides with a *different* existing account

**Not a security hole, but an asymmetry worth documenting.** `OAuthLogin`'s email-collision interstitial (§H.2, §C Scenario 1) exists because an *unauthenticated* request must never be trusted to pick which account to log into by email alone — Planning Center has no `email_verified` claim, and even Google's `email_verified=true` doesn't excuse skipping proof of the *existing* account's password before merging into it. `OAuthLink` is structurally different: the caller is already authenticated (`currentUser` from `CheckAuth`), and the identity always attaches to that account — there is no email-driven account *selection* step to spoof, so the pre-account-takeover vector §H.2 was written to close does not apply here. Nothing needs a password re-prompt to link.

What **is** missing: `OAuthLink` doesn't check whether the just-fetched `identity.Email` matches a *different* `user_profile.email` before writing it into `user_external_identity.provider_email`. Concretely — Account A (authenticated) links a Google identity whose verified email happens to equal Account B's `user_profile.email` (a coincidence, a shared/household inbox, or B simply signing up with an email A also controls a provider identity for). The result: `provider_email` on A's new identity row duplicates B's `user_profile.email`, with no error, warning, or record of the collision anywhere. This creates two independent accounts that both "own" the same real-world email by different code paths (`user_profile.email` vs `user_external_identity.provider_email`), which nothing currently reconciles.

**Why this can still bite later**, given other known gaps in §I.1:
- 🟡 `SearchUserByEmail` / connection-request endpoints (`main.go:151-156`) key on `user_profile.email` only — they won't surface B when someone searches the collided address expecting to find "the account with this email," which could read as a bug rather than the two-tables-of-record split it actually is.
- 🟡 Returning-user profile sync (§I.1) already flags that a changed provider email could collide with `UNIQUE(email)` on `user_profile` directly, on *login* auto-create. `OAuthLink` sidesteps that particular `UNIQUE` constraint (it never touches `user_profile.email`), but the underlying "which account does this email really belong to" ambiguity is the same shape of problem, just deferred to whatever code eventually needs to reconcile `provider_email` against the account table.

**Suggested follow-up (not scheduled):** have `OAuthLink` probe `user_profile` by `LOWER(identity.Email)` the same way `OAuthLogin` does, and when it matches an account other than `currentUser`, either (a) proceed but surface a non-blocking warning to the client (`"This Google account's email is already used by another prayerloop account"`) so the UI can inform the user without blocking the link, or (b) reuse the existing pending-link/password-confirmation mechanism defensively even though the takeover vector doesn't apply — mainly to keep a single, consistent "email collision" code path across Scenarios 1 and 2 rather than two divergent ones. Low priority relative to #39 (Google/Apple providers) and the mobile-side linking UI, since it's a data-hygiene edge case, not a blocker.

**2026-07-09: decided against, not just deprioritized.** Discussed and explicitly ruled out — not merely left unscheduled. A warning/confirmation UX for this case was judged more likely to confuse a user than help one (the realistic trigger is someone who forgot they already had an account and made a second one — on the rare occasion they'd even notice, "oh yeah I have two accounts, I'll just use whichever" is the expected reaction, not a support ticket). Given the compounding low odds (collision occurring × user noticing × user caring enough to act), the cost of building and shipping either (a) or (b) above isn't justified. If this surfaces as an actual support issue, revisit then with a real example in hand rather than speculatively.

---

## Q. 2026-07-09 addendum — backend + DB work complete; only mobile remains

All backend work tracked by the project board (`prayerloop`, EM10Tech) is done. Cross-checked directly against `gh issue list`/`gh project item-list` rather than relying on this doc's own narrative:

- **Issues #33–#39 (all `oauth-integration`-labeled, the entirety of this repo's open-issue list) are code-complete.** #33 (email-collision interstitial), #34 (transaction safety), #35 (delete-account + token revocation), #36 (auth foundation: nullable password, refresh/logout), #37 (PCO login), and #39 (Google + Apple providers) were already committed (see §N/§O). **#38 (OAuthLink/OAuthUnlink/ListUserIdentities/SetPassword)** — the last one still "In Process" on the board — is implemented and test-covered in the current working tree (§P.1), pending only a commit.
- **`prayerloop-psql` (branch `feature/oauth`) is fully committed**, working tree clean: migration `026_add_oauth_account_linking.sql` (`user_external_identity`, `auth_refresh_token`, nullable `user_profile.password`), `027_add_oauth_pending_link.sql` (`oauth_pending_link`), `028_partial_unique_user_profile_email.sql` (partial unique index excluding `''`, needed for multiple no-email OAuth accounts to coexist). All three tables referenced by the backend code (`user_external_identity`, `oauth_pending_link`, `auth_refresh_token`) exist in committed migrations — the "unconfirmed dependency" flagged in §N.3 is resolved.
- The one open, consciously-deferred item is §P.2 (`OAuthLink` doesn't cross-check the provider email against a *different* account's `user_profile.email`) — decided against fixing, not a gap awaiting work (see the addendum immediately above).

**What's left is entirely `prayerloop-mobile`** (a separate repo, not audited line-by-line here, but per §F/§M.4's design): OAuth button UI (`OAuthButtons.tsx`), the PKCE/`expo-auth-session` flow (`util/oauth.ts`), `expo-secure-store`-backed refresh-token storage replacing the raw-password re-login (`util/reLogin.ts`), `expo-apple-authentication` + `@react-native-google-signin/google-signin` native modules (dev-client/EAS build required — no Expo Go), the Linked Accounts settings card (`LinkedAccountsCard.tsx`) for scenario 2/4, and forwarding Apple's one-time `fullName` to the backend (the one still-open item from §M.3/§O against `AppleProvider.FetchIdentity`'s comment that name capture "requires the mobile app to forward it explicitly").
